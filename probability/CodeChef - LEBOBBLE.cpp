...
void Main(const int &T){
    ...
    double ep=0;
    segment_tree<double> A(m),B(m);
    vector<double> x(m+1),y(m+1);
    for(int i=n-1; i>=0; i--){

        double p_chng = p[i]/100.0;
        double p_cnst = (100-p[i])/100.0;

        int num_chng = mp[a[i]+d];
        int num_cnst = mp[a[i]];

        ep+=p_chng*A.query(1,num_chng-1);
        ep+=p_chng*B.query(1,num_chng-1);

        ep+=p_cnst*A.query(1,num_cnst-1);
        ep+=p_cnst*B.query(1,num_cnst-1);

        
        A.update(num_cnst,num_cnst,p_cnst);
        B.update(num_chng,num_chng,p_chng);

    }
    cout<<fixed<<setprecision(4)<<ep<<endl;
}
int main()
{
    file();

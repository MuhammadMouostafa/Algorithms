...
bool valied(int k, int path){
	return k>=path&&k%2==path%2;
}
void Main(const int &T){
	...
	while(q--){
		...
		int u_v_dis=g.get_distance(u,v);
		int u_x_y_v=g.get_distance(u,x)+1+g.get_distance(y,v);
		int u_y_x_v=g.get_distance(u,y)+1+g.get_distance(x,v);

		bool ok=false;

		ok = ok || valied(k,u_v_dis);
		ok = ok || valied(k,u_x_y_v);
		ok = ok || valied(k,u_y_x_v);

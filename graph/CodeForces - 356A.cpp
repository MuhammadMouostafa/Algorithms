...
struct DSU {
	vector<int> rank, parent, size,mx;
	...
	void link(int par, int node, int mxx) {
		if(mxx==-1){
			mxx=max(mx[node],mx[par]);
		}
		mx[node]=mx[par]=mxx;
		parent[node] = par;
		size[par] += size[node];
	...
	bool union_sets(int v, int u, int mxx=-1) {
...
void Main(const int &T){
	...
	while(m--){
		...
		int l=L,r=R;
		while(l<=R){
			r=rieght.mx[rieght.find_set(l)];
			
			int w=winer.mx[winer.find_set(r)];
			
			if(w>=L&&w<=R&&w!=W){
				ans[w]=W;
				winer.union_sets(w,W,W);
			}

			if(r+1<=R){
				rieght.union_sets(r,r+1);
			}

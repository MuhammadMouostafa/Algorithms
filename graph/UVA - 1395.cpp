...
int MST_Kruskal(int n,int str, vector<edge> &edgeList) {//O(edges*log(edges))
	DSU uf(n);
	vector < edge > edges;
	int mx=-1,cnt=0;
	sort(edgeList.begin(), edgeList.end());
	for(int i=str; i<edgeList.size(); i++)
		if (uf.union_sets(edgeList[i].from, edgeList[i].to)) {
			mx= edgeList[i].weight;
			cnt++;
		}
	if (cnt != n - 1)
		return 0;
...
void Main(const int &T){
	...
	while(cin>>n>>m&&(n||m)){
		...
		ll mn=1e9;
		for(int i=0; i<m; i++){
			int mx =MST_Kruskal(n,i,edgeList);
			if(mx){
				mn=min(mn,mx-edgeList[i].weight);
			}
		}

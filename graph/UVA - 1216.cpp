...
double MST_Kruskal(int n, int m,vector<edge> edgeList) {//O(edges*log(edges))
	DSU uf(n);
	vector < edge > edges;
	double sum=0;
	int num=n;
...
ll distance(pii a, pii b){
	return pow(abs(a.first-b.first),2)+pow(a.second-b.second,2);
}
void Main(const int &T){
	...
	for(int i=0; i<n; i++){
		for(int j=i+1; j<n; j++){
			edgeList.push_back(edge(i,j,distance(points[i],points[j])));
		}
	}
	cout<<ceil(MST_Kruskal(n,m,edgeList))<<endl;
}
int main()
{
    file();

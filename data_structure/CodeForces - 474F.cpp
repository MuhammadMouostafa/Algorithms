...
void Main(const int &T){
	...
	sparse_table<Ants> sp(v,[](Ants left, Ants right){
		Ants res;
		res.mn=min(left.mn,right.mn);
		res.g=gcd(left.g,right.g);
		if(left.g==right.g){
			res.cnt=left.cnt+right.cnt;
			res.mults=left.mults+right.mults;
		}else if(res.g==left.g){
			res.cnt=left.cnt;
			res.mults=left.mults+right.mults+right.cnt;
		}else if(res.g==right.g){
			res.cnt=right.cnt;
			res.mults=right.mults+left.mults+left.cnt;
		}else {
			res.cnt=0;
			res.mults=left.cnt+right.cnt+left.mults+right.mults;
		}
		return res;
	});
	...
	while(t--){
		...
		cin>>l>>r;
		l--;r--;
		Ants ants=sp.query_shifting(l,r);
		
		if(ants.mn==ants.g){
			cout<<ants.mults<<endl;
		}else{
			cout<<r-l+1<<endl;
		}
	}
